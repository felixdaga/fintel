...
class FakeCompletion:
    def __init__(self, *, text: str = "", tool_calls=(), finish: str = "stop", model: str = "fake/m") -> None:
        self.text = text
...
class FakeLLM:
    ...
    def complete(self, messages, *, tools=(), force_tool=None, max_tokens=None) -> FakeCompletion:
        ...
        if force_tool == "submit_verification":
            return FakeCompletion(tool_calls=[FakeToolCall("submit_verification", {
                "recommendation": "KEEP", "severity": "none", "corrections": [],
            })])
            ...
            return FakeCompletion(tool_calls=[FakeToolCall("submit_views", {
                "views": [{
                    ...
                    "sources_cited": [
                        ...
                        {"source_type": "fundamentals", "source_id": "2026-03-29", "excerpt": "revenue=215.9B"},
                    ],
                }],
            })])
        if "Quantitative evidence" in user:
            return FakeCompletion(text="quant report\n## key evidence\nprices | 2026-04-23 | close=177.19")
...
def test_decide_one_runs_the_four_stage_pipeline():
    ...
    res = agent.decide_one(symbol="AAPL", trade_date="2026-04-24",
                           quant_evidence="Q", qual_evidence="N")

    assert res.error is None
    assert res.submit_args is not None
    assert [c.stage for c in res.calls] == [
        "quantitative_specialist", "qualitative_specialist",
...
def test_decide_one_skips_the_verifier_when_disabled():
    ...
    res = agent.decide_one(symbol="AAPL", trade_date="2026-04-24",
                           quant_evidence="Q", qual_evidence="N")
    assert [c.stage for c in res.calls] == [
        "quantitative_specialist", "qualitative_specialist", "final_pm",
    ]
...
def test_legacy_string_sources_cited_still_parse():
    payload = {"views": [{"symbol": "AAPL", "score": 0.3, "rationale": "ok",
                          "sources_cited": ["prices", "news"]}]}
...
def test_a_missing_llm_is_a_clear_error_not_a_silent_crash():
    ...
    try:
        agent.decide_one(symbol="AAPL", trade_date="2026-04-24",
                          quant_evidence="Q", qual_evidence="N")
...
def test_pm_not_calling_submit_is_reported_as_an_error():
    class NoSubmit(FakeLLM):
        def complete(self, messages, *, tools=(), force_tool=None, max_tokens=None):
            ...
            return super().complete(messages, tools=tools, force_tool=force_tool, max_tokens=max_tokens)
