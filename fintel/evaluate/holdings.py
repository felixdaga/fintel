...
def active_weights(signal: dict[Symbol, float], *, active_budget: float = DEFAULT_ACTIVE_BUDGET) -> dict[Symbol, float]:
...
def _nav_series(
    ...
    *,
    ...
    for i, d in enumerate(dates):
        ...
        fwd = {
            s: r for s, r in (
                (s, prices.forward_return(s, d, end)) for s in w
            ) if r is not None
        }
        if not fwd:
            prev_w = w
...
def build(signals: Signals, prices: PriceLookup, *, params: dict) -> dict[str, Any] | None:
    ...
    ensemble = _nav_series(
        signals.ensemble, prices, cost_bps=cost_bps, active_budget=active_budget
    )
    per_run = [
