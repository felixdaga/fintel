...
def get_rule(name: str) -> WeightFn:
    if name not in _REGISTRY:
        raise ValueError(f"unknown holdings rule {name!r}; available: {sorted(_REGISTRY)}")
    return _REGISTRY[name]


@register("ew_long_threshold")
def ew_long_threshold(signal: dict[Symbol, float], params: dict[str, Any]) -> dict[Symbol, float]:
...
def score_weighted_long(signal: dict[Symbol, float], params: dict[str, Any]) -> dict[Symbol, float]:
    ...
    return (
        {s: v / total for s, v in longs.items()}
        if total > 0
        else {s: 1.0 / len(longs) for s in longs}
    )


@register("naive_tilt")
def naive_tilt(signal: dict[Symbol, float], params: dict[str, Any]) -> dict[Symbol, float]:
...
def price_weighted(signal: dict[Symbol, float], params: dict[str, Any]) -> dict[Symbol, float]:
